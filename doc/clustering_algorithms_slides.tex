...
\begin{frame}{Two entries that are not clustering algorithms}
  ...
  \begin{block}{Energy recovery --- the ``-ER'' variants}
    ...
    exactly $N$ jets. Applied to the \Cca{C/A} scan only.

    \smallskip
    There is deliberately no $k_T$ equivalent: energy recovery on a collection
    that already has exactly $N$ jets is the identity.
  \end{block}
\end{frame}
