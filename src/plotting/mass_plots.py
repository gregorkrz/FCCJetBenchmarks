...
#
# The main Higgs-mass loop below wraps each file's processing in a try/except:
# an error on one process (bad histogram, unexpected shape, ...) is skipped
# with a warning instead of crashing the whole script and losing
# mass_dashboard_data.pkl (and therefore Higgs mass data in the dashboard)
# for every other process in this method - same failure mode as the one
# fixed in resolution_plots.py.
import os
import ROOT
...
def _histogram_for_dashboard(x_vals, y_vals, max_points=200):
    ...
    Also includes the full-resolution histogram under "edges_full"/"y_full",
    split back out into a separate lazily-loaded file by
    build_dashboard_data.py's _split_full_histograms - not meant to end up
    inlined in dashboard.html itself.
    """
    ...
    return {
        "edges": edges_ds.tolist(), "y": y_ds.tolist(),
        "edges_full": edges.tolist(), "y_full": y_vals.tolist(),
    }
...
for i, fname in enumerate(sorted(root_files)):
    try:
        ...
        print(f"\u26a0\ufe0f Skipping {fname} in Higgs mass plots entirely due to an error: {e}")
        continue
